...
class RMSNorm(nn.Module):
    ...
    def forward(self, x:torch.Tensor, dim=-1):
        ...
        d = x.size(dim=dim)
        rms = (1/d*x.pow(2).sum(dim, keepdim=True)+self.eps)**0.5
...
class RotaryPositionalEmbedding(nn.Module):
    def __init__(self, theta, d_k, max_seq_len, device=None):
        ...
        angles = torch.outer(positions, 1/(theta**(k/d_k)))
    ...
    def forward(self, x:torch.Tensor, token_positions:torch.Tensor):
        ...
        even = pairs[..., 0]
        odd = pairs[..., 1]
        even_new = cos*even - sin*odd
        odd_new = sin*even + cos*odd
        pair_new = torch.stack((even_new, odd_new), dim=-1)
        return rearrange(pair_new, "... d two -> ... (d two)")
...
class TransformerLM(nn.Module):
    def __init__(self, d_model, d_ff, num_heads, num_layers, context_length, vocab_size, rope=None, device=None, dtype=None):
        super().__init__()
