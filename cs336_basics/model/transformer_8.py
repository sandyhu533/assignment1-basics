...
class Embedding(nn.Module):
    def __init__(self, num_embedding, embedding_dim, device=None):
        super().__init__()
        weight = torch.zeros((num_embedding, embedding_dim), device=None)
...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def forward(self, x:torch.Tensor, token_positions:torch.Tensor):
        ...
        even_new, odd_new = cos*even-sin*odd, sin*even+cos*odd
        new_pair = torch.stack([even_new, odd_new], dim=-1)
        return rearrange(new_pair, "... d two -> ... (d two)")

def softmax(x:torch.Tensor, dim=-1):
    xmax = x.amax(dim=dim, keepdim=True)
    xexp = (x-xmax).exp()
    return xexp / xexp.sum(dim=dim, keepdim=True)
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x:torch.Tensor, token_positions=None):
        ...
        c  = q.size(-2)
        mask = torch.tril(torch.ones((c, c), dtype=bool, device=q.device))
        qv = scaled_dot_product_attention(q, k, v, mask)
        att = rearrange(qv, "... num_heads context_length d_k -> ... context_length (num_heads d_k)")
