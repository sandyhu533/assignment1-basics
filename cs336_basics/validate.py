...
from cs336_basics.model import TransformerLM, AdamW, get_lr_learning_rate, gradient_clipping
...
def main():
    ...
    parser.add_argument(
        "--overfit_test",
        action="store_true",
        help="Overfit test: use one small batch and never refresh it. Loss should drop toward 0 if training is correct.",
    )
    parser.add_argument(
        "--save_every",
        type=int,
    ...
    if args.overfit_test:
        # One small slice: same batch every step so loss should go to ~0 if training works.
        overfit_data = data[0 : args.context_length + args.batch_size]
        inputs, labels = get_batch(
            overfit_data, args.batch_size, args.context_length, args.device
        )
    else:
        inputs, labels = get_batch(
    ...
    for _ in range(args.train_steps):
        ...
        # GPT-style: logits at position j predict next token (labels[b,j]); use positions 0..L-2 only (L-1 predictions)
